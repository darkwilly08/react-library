...
export const useConfig = () => {
  ...
  const [confirmPasswordField, setConfirmPassword] = useState<Field>({
    value: '',
    error: null,
  });
  ...
  const passwordMatchValidator = (confirmPassword: string, password: string) => {
    if (!confirmPassword) {
      return 'This field is mandatory';
    }
  ...
  const handleConfirmPasswordChange = (value: string) => {
    setConfirmPassword({
      value,
      error: passwordMatchValidator(value, passwordField.value),
    });
  };
